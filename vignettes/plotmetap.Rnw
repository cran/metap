...
\twograph{plotmetap-plotp}{Q--Q plot from \func{plotp}}{plotp}{plotmetap-plotfunc}{Q--Q plot an object of class \func{metap}}{plotfunc}{Plots of validity data}{plotvalid}
...
that these are drawn iid from a uniform.
...
and for backwards compatibility it is what is used
it you call the \func{plot} method
on the objects returned by the analysis functions.
It is also possible to produce this plot by setting
the \func{plotversion} parameter to "old" in the call
to \func{plotp}.
An example is shown in Figure \ref{plotfunc}
which first calls \func{sumlog}.
Eventually the plot method will use the new version but the
legacy one will always be an option.
...
the plotting function and the documentation for
the \func{qqconf} package should be consulted for details.
The package vignette is also very helpful.
